...
struct RetryDecisionTests {
    ...
    func getServerErrorRetries() {
        ...
        #expect(
            RetryDecision.evaluate(policy: policy, method: .get, attempt: 1, error: .server(context(503)))
                == .retry(after: 0.5))
        #expect(
            RetryDecision.evaluate(policy: policy, method: .get, attempt: 2, error: .server(context(503)))
                == .retry(after: 1))
    ...
    func postNotRetried() {
        #expect(
            RetryDecision.evaluate(policy: noJitter, method: .post, attempt: 1, error: .server(context(503))) == .stop)
    }
    ...
    func postRetriesWhenOptedIn() {
        ...
        #expect(
            RetryDecision.evaluate(policy: policy, method: .post, attempt: 1, error: .server(context(503)))
                == .retry(after: 0.5))
    }
    ...
    func nonRetryableStatus() {
        ...
        #expect(
            RetryDecision.evaluate(policy: noJitter, method: .get, attempt: 1, error: .server(context(501))) == .stop)
        #expect(
            RetryDecision.evaluate(policy: noJitter, method: .get, attempt: 1, error: .notFound(context(404))) == .stop)
    }
    ...
    func rateLimited() {
        ...
        #expect(
            RetryDecision.evaluate(
                policy: policy, method: .get, attempt: 1, error: .rateLimited(retryAfter: 2, context(429)))
                == .retry(after: 2))
        ...
        #expect(
            RetryDecision.evaluate(
                policy: capped, method: .get, attempt: 1, error: .rateLimited(retryAfter: 999, context(429)))
                == .retry(after: 5))
        ...
        #expect(
            RetryDecision.evaluate(
                policy: ignore, method: .get, attempt: 1, error: .rateLimited(retryAfter: 999, context(429)))
                == .retry(after: 0.5))
    }
    ...
    func transportErrorsRetry() {
        ...
        #expect(
            RetryDecision.evaluate(policy: policy, method: .get, attempt: 1, error: .noInternet) == .retry(after: 0.5))
        #expect(
            RetryDecision.evaluate(
                policy: policy, method: .get, attempt: 1, error: .transport(underlying: URLError(.dnsLookupFailed)))
                == .retry(after: 0.5))
        #expect(
            RetryDecision.evaluate(
                policy: policy, method: .get, attempt: 1, error: .transport(underlying: URLError(.badURL))) == .stop)
    }
